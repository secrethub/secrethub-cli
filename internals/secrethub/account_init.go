...
func (cmd *AccountInitCommand) Register(r command.Registerer) {
	...
	clause.BoolVarP(&cmd.useClipboard, "clip", "c", false, "Copy the credential's public component to the clipboard instead of printing it to stdout.")
	clause.BoolVar(&cmd.noWait, "no-wait", false, "Do not hang waiting for the credential's public component to be added to the account and instead exit after outputting the credential's public component. To finish initializing the account, use the --continue flag after adding the credential to the account.")
	clause.BoolVar(&cmd.isContinue, "continue", false, "Continue initializing the account. Use this when a credential has already been generated by a previous execution of the command.")
