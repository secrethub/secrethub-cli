...
func (cmd *RunCommand) Register(r command.Registerer) {
	...
	clause.BoolVar(&cmd.noMasking, "no-masking", false, "Disable masking of secrets on stdout and stderr")
	clause.BoolVar(&cmd.maskerOptions.DisableBuffer, "no-output-buffering", false, "Disable output buffering. This increases output responsiveness, but decreases the probability that secrets get masked.")
