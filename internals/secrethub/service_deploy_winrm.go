...
func (cmd *ServiceDeployWinRmCommand) Register(r command.Registerer) {
	...
	clause.StringVar(&cmd.authType, "auth-type", "basic", "Authentication type (basic/cert)")
	...
	clause.StringVar(&cmd.username, "username", "", "The username used for logging in when authentication type is basic. Is asked if not supplied.")
	clause.StringVar(&cmd.password, "password", "", "The password used for logging in when authentication type is basic. Is asked if not supplied.")
	clause.StringVar(&cmd.clientCert, "client-cert", "", "Path to client certificate used for certificate authentication.")
	clause.StringVar(&cmd.clientKey, "client-key", "", "Path to client key used for certificate authentication.")
	clause.StringVar(&cmd.caCert, "ca-cert", "", "Path to CA certificate used to verify server TLS certificate.")
	clause.BoolVar(&cmd.noVerify, "insecure-no-verify-cert", false, "Do not verify server TLS certificate (insecure).")
