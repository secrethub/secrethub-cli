...
func (cmd *ServiceGCPInitCommand) PreRun(c *cobra.Command, args []string) error {
	var err error
...
func (cmd *ServiceGCPInitCommand) Register(r command.Registerer) {
	...
	clause.StringVar(&cmd.kmsKeyResourceID, "kms-key", "", "The Resource ID of the KMS-key to be used for encrypting the service's account key.")
	clause.StringVar(&cmd.serviceAccountEmail, "service-account-email", "", "The email of the GCP Service Account that should have access to this service account.")
