	...
	if err == unix.ENOSYS {
		return errMlock.Code("enosys").Errorf(
			"%s\n\n"+
				...
				"Please enable mlock on your system, try again with root priviliges,",
			err)
		...
		return errMlock.Code("enomem").Errorf(
			"%s\n\n"+
				...
				"Either execute as root, or give your user the right priviliges by editing the /etc/security/limits.conf file.\n"+
		...
		return errMlock.Code("eperm").Errorf(
			"%s\n\n"+
				...
				"This usually means that you need root priviliges.\n"+
				"Please try again with root priviliges and see if the problem persists.",
			err)
	} else if err != nil {
